...
function Info:draw()
  ...
  if self.level == 1 then
    love.graphics.draw(corn, getCenteredHorizontalPosition(300), 450)
  ...
  elseif self.level == 2 then
    ...
    love.graphics.printf(titleText, getCenteredHorizontalPosition(width), 100, width, "left")
    love.graphics.setFont(love.graphics.newFont(FONT_SECOND, 16))
    local levelText = "'Mart' is the bulgarian word for March and 'baba' - the word for grandma. In old folklore 'Baba Marta' is portrayed as a volatile and moody woman. It is believed that when she is happy, skies are blue and the sun will shine, but when Baba Marta is disgruntled, she will bring rain and wind. Baba Marta only visits clean and tidy homes, according to the legends, and spring-cleaning is another part of the Baba Marta celebration. At the end of February, people clean their homes in preparation. \n\nFolklore also states that Baba Marta likes to see young girls on 1st of March, so old people would stay indoors in order not to offend her."
    love.graphics.printf(levelText, getCenteredHorizontalPosition(width), 170, width, "left")
  elseif self.level == 3 then
    love.graphics.setFont(love.graphics.newFont(FONT, 24))
